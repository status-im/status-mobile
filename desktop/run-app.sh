...
if [[ "$executor" == "ReactNetExecutor" ]]; then
  (node /home/max/work/status-react-desktop/status-react1/status-react/desktop/bin/ubuntu-server.js 2>&1 > /dev/null) &
fi

if [[ $on_device == 1 ]]; then
  app_path="/home/phablet/StatusIm"

# Push binaries
  adb push /home/max/work/status-react-desktop/status-react1/status-react/desktop/bin/StatusIm "$app_path/StatusIm"
...
# Run app on device
  adb shell "cd $app_path && REACT_SERVER_HOST=$react_host ./StatusIm --host $react_host $args -- --desktop_file_hint=/usr/share/applications/webbrowser-app.desktop"
else
# Run app locally
  /home/max/work/status-react-desktop/status-react1/status-react/desktop/bin/StatusIm $args
fi
