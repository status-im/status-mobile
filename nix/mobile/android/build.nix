...
let
  ...
  nwakuAndroid = (builtins.getFlake "github:waku-org/nwaku").packages.${builtins.currentSystem}.default;
...
in stdenv.mkDerivation rec {
  ...
  postUnpack = ''
    ...
    # Copy nwaku library in target format
    TEMP_DIR=$(mktemp -d)
    mkdir $TEMP_DIR/jni
    cp -R ${nwakuAndroid}build/android/${androidAbiInclude}/ $TEMP_DIR/jni/.
