...
(defn label-component
  ...
  (rn.pure/view
   {:accessibility-label :label-component}
   (case label
     :text    (text/text
               {:style (style/color blur? theme)}
               label-props)
     ...
     :preview (reagent/as-element
               [preview-list/view {:type (:type label-props) :size :size-24}
                (:data label-props)])
     nil)))
...
(defn action-component
  ...
  (rn.pure/view
   ...
   (case action
     :arrow    (icon/icon (or (:icon action-props) :i/chevron-right)
                          (style/color blur? theme))
     ...
     :selector (reagent/as-element [selectors/view action-props])
     nil)))
...
(defn view
  ...
  (rn.pure/pressable
   ...
   (rn.pure/view
    {:style (style/left-sub-container props)}
   ...
   (reagent/as-element [tag-component props])
   (rn.pure/view
    {:style (style/sub-container (:alignment action-props))}
