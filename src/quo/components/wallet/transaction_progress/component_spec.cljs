...
(defn- get-test-data
  [{:keys [state network] :or 
    {state :pending network :mainnet}}]
  {:title "Title"
   :counter (reagent/atom 0)
   :total-box 85
   :accessibility-label :transaction-progress
   :tag-name "Doodle"
   :tag-number "120"
...
(h/describe "Transaction Progress"
            ...
            (h/test "arbitrum progress box is visible network is optimism-arbitrum"
                    (h/render [quo/transaction-progress (get-test-data {:network :optimism-arbitrum})])
                    (h/is-truthy (h/get-all-by-label-text :arbitrum-progress-box)))
