       ...
       [rn/view
        ...
        [rn/view
         ...
         (when icon
           [icon/icon
            icon
            (cond-> {:size (if (= size :large) 20 12)}
              icon-color       (assoc :color icon-color)
              (not icon-color) (assoc :no-color true))])]
        [text/text
         {:size   paragraph-size
          :weight :medium
...
(defn- pending
  [size theme label blur? no-icon? container-style icon icon-color]
  [base-tag
...
(defn status-tag
  [{:keys [status size label blur? no-icon? icon container-style icon-color]}]
  (when status
