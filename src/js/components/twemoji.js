...
const TwemojiImage = ({ style, rawText, iconId }) => {
  const [errorInFetching, setErrorInFetching] = React.useState(false);

  const source = getTwemojiUrl(iconId);
...
const parseString = (text, style) => {
  ...
  while ((match = EMOJI_REGEX.exec(text))) {
     ...
     */
    const key = `twemoji-${iconId}-${textIndex}-${randomNumber}`;

    result.push(<TwemojiImage key={key} style={style} rawText={rawText} iconId={iconId} />);
    modified = true;
  }
