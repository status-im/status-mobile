(ns quo2.components.inputs.input.component-spec
  (:require [quo2.components.inputs.input.view :refer [input]]
...
(h/describe "Input"
  ...
  (h/describe "Icon"
    ...
    (h/test "Renders"
      (h/render [input {:icon-name :i/placeholder}])
  ...
  (h/describe "Label"
    ...
    (h/test "Renders with specified text"
      (let [input-label "My label"]
        (h/render [input {:label input-label}])
  ...
  (h/test "Char limit counter"
    (let [char-limit     100
      ...
      (h/render [input {:char-limit char-limit}])
      (h/is-truthy (h/query-by-label-text :input-labels))
