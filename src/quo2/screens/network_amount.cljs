(ns quo2.screens.network-amount
  (:require [quo.previews.preview :as preview]
            [quo.react-native :as rn]
            [quo2.components.network-amount :refer [network-amount]]
...
(defn- networks->options [networks]
  (for [{:keys [name] :as network} networks]
    {:key network
     :value name}))
...
(defn- state->opts [{:keys [eth-value network]
