...
(rf/defn delete-message [cofx id]
  (delete-message-rpc id))

(rf/defn delete-messages-from [cofx author]
  (delete-messages-from-rpc author))

(rf/defn mark-messages-seen [cofx chat-id ids on-success]
  (mark-seen-rpc chat-id ids on-success))

(rf/defn delete-messages-by-chat-id [cofx chat-id]
