...
(def translations
  {
   ...
   :type-a-command                        "Inizia scrivendo un comando..."
   :error                                 "Errore"
   ...
   :camera-access-error                   "Per concedere i permessi necessari per la telecamera, per favore vai nelle impostazioni di sistema e assicurati che la voce Status > Camera sia selezionata."
   ...
   :phone-number-required                 "Tocca qui per convalidare il tuo numero di telefono e troveremo i tuoi amici."
   ...
   :debug-enabled                         "Il server di debug è stato lanciato! Adesso puoi eseguire *status-dev-cli scan* per trovare dal tuo computer il server nella tua stessa rete."
   ...
   :confirmations-helper-text             "Per favore aspetta almeno 12 conferme per essere sicuro che la tua transazione è stata processata in sicurezza"
   ...
   :testfairy-message                     "Stai usando un app installata da una build nightly. Per scopi di testing questa build include sessioni che registrano se è usata la connessione wifi, così tutte le tue interazioni con l'app sono salvate (sia video che log) e possono essere usate dal tuo team di sviluppo per investigare possibili issue. I video/log salvati non includono le tue password. La registrazione è fatta solo se l'app è installata da una build nightly. Niente è registrato se l'app è installata dal PlayStore o da TestFlight."
   ...
   :wallet-total-value                    "Valore total"
   :wallet-settings                       "Impostazioni del portafoglio"
   :signing-phrase-description            "Firma la transazione inserendo la tua password. Assicurati che le parole sopra siano uguali alla tua signing phrase segreta"
   ...
   :transactions-unsigned                 "Non firmato"
   ...
   :transactions-filter-select-all        "Seleziona tutti"
   :view-transaction-details              "Vedi i dettagli della transazione"
   :transaction-description               "Per favore aspetta almeno 12 conferme per essere sicuro che la tua transazione è stata processata in sicurezza"
   ...
   :validation-amount-is-too-precise      "L'ammontare è troppo preciso. La più piccola unità che tu possa mandare è 1 Wei (1x10^-18 ETH)"
   ...
   :edit-network-warning                  "Sta attento, modificando i dati della rete puoi disabilitare questa rete per te"
   :connecting-requires-login             "La connessione ad un altra rete richiede il login"
   :close-app-title                       "Attenzione!"
   :close-app-content                     "L'app si fermerà e terminerà. Quando la riaprirai, verrà utilizzata la rete selezionata"
