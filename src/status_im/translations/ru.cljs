...
(def translations
  {
   ...
   :camera-access-error                   "Чтобы предоставить необходимое разрешение для камеры, пожалуйста, перейдите в настройки системы и убедитесь, что выбрано Статус > Камера."
   :photos-access-error                   "Чтобы предоставить необходимое разрешение для фото, пожалуйста, перейдите в настройки системы и убедитесь, что выбрано Статус > Камера."
   ...
   :status-prompt                         "Укажите свой статус. Использование #хэштеги поможет другим узнать о вас и рассказать о том, что у вас на уме"
   ...
   :here-is-your-signing-phrase           "Вот ваша фраза подписи. Вы будете использовать ее для подтверждения своих транзакций. *Запишите ее и храните в безопасности!*"
   :phone-number-required                 "Нажмите сюда для ввода своего номера телефона и я найду ваших друзей."
   :shake-your-phone                      "Найдена ошибка или есть предложение? Просто ~встряхните~ свой телефон!"
   ...
   :move-to-internal-failure-message      "Нам нужно переместить некоторые важные файлы из внешнего хранилище во внутреннее. Для этого нам нужно ваше разрешение. В следующих версиях мы не будем использовать внешнее хранилище."
   :debug-enabled                         "Запущен сервер отладки! Теперь вы можете выполнить * status-dev-cli scan *, чтобы найдите сервер с вашего компьютера в той же сети."
   ...
   :no-statuses-discovered-body           "Когда кто-то отправляет\nстатус, вы увидите его здесь."
   ...
   :delete-contact-confirmation           "тот контакт будет удален из ваших контактов"
   ...
   :cost-fee                              "Стоимость/Комиссионные"
   :nonce                                 "Нонс"
   ...
   :open-on-etherscan                     "Открыть ссылку Etherscan.io"
   ...
   :testfairy-message                     "Вы используете приложение, установленное из ночной сборки. Для целей тестирования эта сборка включает в себя запись сеанса, если используется соединение Wi-Fi, так что все ваши взаимодействия с этим приложением сохраняется (как видео и журналы) и могут использоваться нашей командой разработчиков для изучения возможных проблем. Сохраненные видео / журналы не включают в себя ваши пароли. Запись выполняется только в том случае, если приложение установлено из ночной сборки. Ничего не записывается, если приложение установлено из PlayStore или TestFlight."
   ;; wallet
   :wallet                                "Кошелек"
   :wallets                               "Кошельки"
   ...
   :transactions-delete-content           "Транзакция будет удалена из списка «Без Подписей»"
   :transactions-history                  "История"
   :transactions-unsigned                 "Без Подписей"
   ...
   :transaction-moved-text                "Трансакция будет находится в списке «Без Подписей» в течение 5 минут"
   :transaction-moved-title               "Транзакция перемещена"
   :sign-later-title                      "Подписать транзакцию позже?"
   :sign-later-text                       "Проверить историю транзакций, чтобы подписать эту транзакцию"
   ...
   :edit-network-warning                  "Будьте осторожны, редактирование сетевых данных может отключить эту сеть для вас"
