...
(def translations
  {
   ...
   :browsing-open-in-web-browser          "Відрити у веб-браузері"
   ...
   :testfairy-message                     "Ви використовуєте додаток з 'нічних збірок'. З метою тестування ця збірка включає в себе запис сесії, якщо активне з'єднання Wi-Fi, тому всі ваші взаємодії з цим додатком зберігаються (як відео чи журнали) і можуть використовуватися нашою командою розробників для вивчення можливих проблем. Збережені відео / журнали не включають ваші паролі. Запис виконується лише в тому випадку, якщо додаток встановлено з нічного збірки. Нічого не записується, якщо додаток встановлено з PlayStore або TestFlight."
   ...
   :signing-phrase-description            "Введіть свій пароль, щоб підписати транзакцію. Переконайтеся, що наведені вище слова співпадають з вашою кодовою фразою-підписом"
   ...
   :transaction-moved-text                "Транзакція залишиться у списку 'Непідписані' протягом наступних 5 хвилин"
