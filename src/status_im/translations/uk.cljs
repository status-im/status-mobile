...
(def translations
  {
   ...
   :photos-access-error                   "Щоб надати необхідний дозвіл знімати фото, pперейдіть до налаштувань вашої системи та переконайтеся, що Status > Photos є вибране."
   ...
   :phew-here-is-your-passphrase          "Фух, це було важко. Ось ваша кодова фраза, *запишіть її та надійсно збережіть!* Вона пригодиться для відновлення аккануту."
   :here-is-your-passphrase               "Ось ваша кодова фраза, *запишіть її та надійсно збережіть!* Вона пригодиться для відновлення аккануту."
   :here-is-your-signing-phrase           "Ось ваша фраза-підпис. Ви будете використовувати це для підтвердження ваших транзакцій. *запишіть її та надійсно збережіть!*"
   ...
   :edit-contacts                         "Резагувати контакти"
   ...
   :enter-password                        "Введіть password"
   ...
   :testfairy-message                     "Ви використовуєте додаток з 'нічних зіброк'. З метою тестування ця збірка включає в себе запис сесії, якщо активне з'єднання WiFi, тому всі ваші взаємодії з цим додатком зберігаються (як відео чи журнали) і можуть використовуватися нашою командою розробників для вивчення можливих проблем. Збережені відео / журнали не включають ваші паролі. Запис виконується лише в тому випадку, якщо додаток встановлено з нічного збірки. Нічого не записується, якщо додаток встановлено з PlayStore або TestFlight."
   ...
   :wallet-settings                       "Наташтування гаманця"
   ...
   :existing-networks                     "Інуючі мережі"
