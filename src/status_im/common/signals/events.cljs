...
(rf/defn process
  ...
  (let [^js data     (.parse js/JSON event-str)
    ...
    (case type
      ...
      "envelope.sent" (messages.transport/update-envelopes-status
                       cofx
      ...
      "connector.sendRequestAccounts"
      {:fx [[:dispatch
             [:browser.rpc/on-request-accounts-signal
              (->> event-js
                   transforms/js->clj
                   (cske/transform-keys transforms/->kebab-case-keyword))]]]}

      "connector.dAppPermissionGranted"
      {:fx [[:dispatch
             [:browser.rpc/on-permission-granted-signal
              (->> event-js
                   transforms/js->clj
                   (cske/transform-keys transforms/->kebab-case-keyword))]]]}

      "connector.dAppChainIdSwitched"
      {:fx [[:dispatch
             [:browser.rpc/on-chain-switched-signal
              (->> event-js
                   transforms/js->clj
                   (cske/transform-keys transforms/->kebab-case-keyword))]]]}

      "connector.dAppPermissionRevoked"
      {:fx [[:dispatch
             [:browser.rpc/on-permission-revoked-signal
              (->> event-js
                   transforms/js->clj
                   (cske/transform-keys transforms/->kebab-case-keyword))]]]}
      ...
      {:fx [[:dispatch
             [:browser.rpc/on-transaction-signal
              (->> event-js
                   transforms/js->clj
                   (cske/transform-keys transforms/->kebab-case-keyword))]]]}
