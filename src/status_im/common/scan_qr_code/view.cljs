(ns status-im.common.scan-qr-code.view
  (:require [oops.core :as oops]
            ...
            [react-native.vision-camera :as vision-camera]
...
(defn- find-valid-qr-value
  [validate-fn scanned-values]
  (some (fn [{:keys [value]}]
          (when (or (nil? validate-fn)
                    (validate-fn value))
            value))
        scanned-values))

(defn- check-qr-code-and-navigate
  [{:keys [event validate-fn on-success-scan on-failed-scan]}]
  (let [scanned-value (->> event
                           transforms/js->clj
                           (find-valid-qr-value validate-fn))]
    (if scanned-value
      (on-success-scan scanned-value)
      (on-failed-scan))))

(defn- render-camera
  [{:keys [torch-mode qr-view-finder validate-fn error-message set-qr-code-succeeded]}]
  (let [theme              (quo.theme/use-theme)
        scan-code?         (rn/use-ref-atom true)
        set-rescan-timeout (rn/use-callback
                            (fn []
                              (reset! scan-code? false)
                              (js/setTimeout #(reset! scan-code? true) 3000))
                            [])
        on-failed-scan     (rn/use-callback
                            (fn []
                              (set-rescan-timeout)
                              (debounce/debounce-and-dispatch [:toasts/upsert
                                                               {:type  :negative
                                                                :theme theme
                                                                :text  error-message}]
                                                              300))
                            [theme])
        camera-device      (vision-camera/use-camera-device :back)
        code-scanner       (vision-camera/use-code-scanner (fn [event]
                                                             (when @scan-code?
                                                               (check-qr-code-and-navigate
                                                                {:event           event
...
(defn view
  ...
  (let [insets           (safe-area/get-insets)
    ...
    (fn []
      (let [flashlight-icon       (if @torch? :i/flashlight-on :i/flashlight-off)
