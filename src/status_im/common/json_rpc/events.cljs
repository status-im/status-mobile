    ...
    (native-module/call-private-rpc
     ...
     (fn [raw-response]
       (if (string/blank? raw-response)
         (handle-rpc-response on-error deferred p/reject! {:message "Blank response"})
         (let [^js response-js (transforms/json->js raw-response)]
           (if-let [error (.-error response-js)]
             (handle-rpc-response on-error deferred p/reject! (transforms/js->clj error))
             (let [result (if js-response
                            (.-result response-js)
                            (transforms/js->clj (.-result response-js)))]
               (handle-rpc-response on-success deferred p/resolve! result)))))))
    deferred))
