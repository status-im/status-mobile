(ns status-im.subs.wallet.swap
  (:require [clojure.string :as string]
            ...
            [status-im.contexts.wallet.account.core :as account]
...
(rf/reg-sub
 :wallet/swap-enough-native-balance?
 :<- [:wallet/swap-proposal]
 :<- [:wallet/current-viewing-account]
 (fn [[swap-proposal account]]
   (when swap-proposal
     (let [{:keys [from required-native-balance]}    swap-proposal
           {:keys [native-currency-symbol chain-id]} from
           native-token                              (account/get-token account native-currency-symbol)
           native-token-balance                      (utils/token-balance-for-network native-token
                                                                                      chain-id)]
