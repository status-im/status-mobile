...
(defn show-confirmation
  ...
  ([{:keys [ios-confirm-style] :or {ios-confirm-style "destructive"}} title content confirm-button-text on-accept on-cancel]
   (.alert (.-Alert rn-dependencies/react-native)
           ...
           (clj->js (vector (merge {:text                (i18n/label :t/cancel)
                                    :style               "cancel"
                                    :accessibility-label :cancel-button}
...
(defn show-question
  ...
  ([title content on-accept on-cancel]
   (.alert (.-Alert rn-dependencies/react-native)
           title
           content
           (clj->js (vector (merge {:text                (i18n/label :t/no)
...
;; running timeouts
(re-frame/reg-fx :utils/dispatch-later
                 (fn [params]
...
(defn distinct-by-group
  ...
  ([m grouper optimizing ordering]
   (into {} (map (fn [[dup map-elts]]
                   (first (sort-by #(optimizing (second %)) ordering map-elts)))
                 (group-by #(grouper (second %)) m))))
