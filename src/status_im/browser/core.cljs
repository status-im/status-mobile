...
(rf/defn update-bookmark
  ...
  (let [old-bookmark (get-in db [:bookmarks/bookmarks url])
    ...
    (rf/merge cofx {:db            (assoc-in db [:bookmarks/bookmarks url] new-bookmark)
...
(rf/defn delete-bookmark
  ...
  (let [old-bookmark (get-in db [:bookmarks/bookmarks url])
    ...
    (rf/merge cofx {:db            (update db :bookmarks/bookmarks dissoc url)
...
(rf/defn handle-no-permissions [cofx {:keys [method id]} message-id]
