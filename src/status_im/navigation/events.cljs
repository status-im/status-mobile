...
(rf/defn show-bottom-sheet
  ...
  (let [theme                  (or (:theme content) (:theme db))
        ;; NOTE: if true, allows to return the bottom-sheet that was being shown after the stacked
        ;; one is hidden
        {:keys [stacked?]}     content
        {:keys [sheets hide?]} (:bottom-sheet db)]
    (rf/merge cofx
              {:db               (if stacked?
                                   (update-in db [:bottom-sheet :sheets] concat [content])
                                   (update-in db [:bottom-sheet :sheets] conj content))
               :dismiss-keyboard nil}
              (fn [new-cofx]
                (when-not hide?
                  (if (and (not stacked?) (seq sheets))
                    (hide-bottom-sheet new-cofx)
                    {:show-bottom-sheet {:theme theme}}))))))

;; (rf/defn show-nested-bottom-sheet
;;   {:events [:show-nested-bottom-sheet]}
;;   [{:keys [db] :as cofx} content]
;;   (let [theme                  (or (:theme content) (:theme db))
;;         {:keys [sheets hide?]} (:bottom-sheet db)]
;;     (rf/merge cofx
