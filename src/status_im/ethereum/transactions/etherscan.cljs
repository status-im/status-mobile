...
(defn- etherscan-token-transactions
...
(re-frame/reg-fx
 :ethereum.transactions/get-etherscan-transactions
 (fn [{:keys [chain chain-tokens address on-success on-error chaos-mode?]}]
   (etherscan-transactions chain address on-success on-error chaos-mode?)
   (etherscan-token-transactions chain address chain-tokens on-success on-error chaos-mode?)))
