...
(defn right-pad [x]
  (let [len (count x)
    ...
    (if (= 64 to-pad)
      x
      (.rightPad ^js (utils) x (+ len to-pad)))))
...
(defn left-pad [x]
  (let [len (count x)
    ...
    (if (= 64 to-pad)
      x
      (.leftPad ^js (utils) x (+ len to-pad)))))

(defn to-two-complement [x]
  (when x
    (subs (.toTwosComplement ^js (utils) x) 2)))

(defn from-utf8 [x]
  (when x
    (subs (.fromUtf8 ^js (utils) x) 2)))
...
(defn bytes-to-hex [x]
  (when x
    (subs (.bytesToHex ^js (utils) x) 2)))

(defn number-to-hex [x]
  (when x
    (subs (.numberToHex ^js (utils) x) 2)))

(defn hex-to-utf8 [x]
  (.hexToUtf8 ^js (utils) (str "0x" x)))

(defn hex-to-number [x]
  (when x
    (let [hex-x (str "0x" x)]
      (try
        (.hexToNumber ^js (utils) hex-x)
        (catch :default err
          (.hexToNumberString ^js (utils) hex-x))))))
