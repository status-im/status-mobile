...
(def profile-icon-options
  [{:label  (i18n/label :t/image-source-gallery)
   ...
   {:label  (i18n/label :t/image-source-make-photo)
    :action (fn []
              (status-im.thread/dispatch [:request-permissions {:permissions [:camera :write-external-storage]
                                                        :on-allowed  #(status-im.thread/dispatch [:navigate-to :profile-photo-capture])
                                                        :on-denied   (fn []
                                                                       (utils/set-timeout
...
(defn- show-qr [contact source value]
  #(status-im.thread/dispatch [:navigate-to-modal :profile-qr-viewer {:contact contact
                                                              :source  source
                                                              :value   value}]))
