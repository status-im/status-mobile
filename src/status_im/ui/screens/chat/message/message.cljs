...
(defn message-timestamp
  [t justify-timestamp? outgoing command? content]
  (when-not command?
...
(defn message-delivery-status
  ...
  (case status
    ...
    :network-mismatch     [command-network-mismatch-status]
    :not-seen-by-everyone [group-message-delivery-status message-id delivery-recipients delivery-count-label]
    (when status
      [text-status status])))
...
(defn chat-message
  [{:keys [message-id outgoing group-chat modal? current-public-key content-type content] :as message}]
  [react/view
