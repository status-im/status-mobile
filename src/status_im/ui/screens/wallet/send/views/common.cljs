...
(defn fetch-optimal-gas [web3 tx-atom all-tokens chain cb]
     ...
     (fn [_ gas-price]
       (when gas-price
         (ethereum/estimate-gas-web3
          web3
          (clj->js (events/get-tx-params (assoc @tx-atom :value 0) symbol coin))
          (fn [_ gas]
            (when gas
              (cb {:optimal-gas       (money/bignumber (* 2 gas))
...
(defn refresh-optimal-gas [web3 tx-atom all-tokens chain]
  (fetch-optimal-gas web3 tx-atom all-tokens chain
                     (fn [res]
