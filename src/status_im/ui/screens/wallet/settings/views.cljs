...
(defn- render-token-wrapper [items]
  (fn [_ item]
    (let [{:keys [symbol name icon color checked?] :as token} (get items item)]
      (reagent/as-element
...
(defn manage-assets []
  (let [{search-filter                           :search-filter
         {default-tokens nil} :tokens} @(re-frame/subscribe [:wallet/filtered-grouped-chain-tokens])]
    [react/view (merge components.styles/flex {:background-color colors/white})
     ...
     [react/view {:style components.styles/flex}
      ...
      [list/fast-list {:data      default-tokens
       ...
       ;;   :sections                    [{:title (i18n/label :t/default)
       ;;                                  :data default-tokens}]
       ;;   :key-fn                      :address
