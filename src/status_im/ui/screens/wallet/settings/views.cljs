...
(defn render-token
  ...
  (let [checked-atom? (reagent/atom checked?)]
      ...
      (fn [{:keys [symbol name icon color custom? checked?] :as token}]
        [list/list-item-with-checkbox
         {:checked?        @checked-atom?
          ...
          :on-value-change
          (fn [val]
            (reset! checked-atom? val)
            (re-frame/dispatch
             [:wallet.settings/toggle-visible-token (keyword symbol) val]))}
         [list/item
          (if icon
...
(defview manage-assets []
  (let [{custom-tokens true default-tokens nil}
        @(re-frame/subscribe [:wallet/grouped-chain-tokens])]
    (fn []
