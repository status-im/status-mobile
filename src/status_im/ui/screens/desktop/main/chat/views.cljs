...
(defn messages-view [messages]
  [react/view {:style styles/messages-view}
   [list/flat-list {:data                messages
                    ...
                    #_#_:footerWidth         styles/messages-list-vertical-padding
...
(defn reply-message
  ...
  [react/view {:style styles/reply-content-container}
   ...
   [react/text {:style styles/reply-content-message} text]])
...
(views/defview chat-view []
  (views/letsubs [{:keys [current-input-text chat-id group-chat messages] :as current-chat} [:chats/current]
                  current-input-text [:chats.ui/input-text]
    ...
    [react/view {:style styles/chat-view}
     ...
     [chat-text-input chat-id current-input-text network-status]]))
...
(views/defview chat-profile
  []
  (views/letsubs [{:keys [pending? public-key] :as contact} [:contacts/current]]
