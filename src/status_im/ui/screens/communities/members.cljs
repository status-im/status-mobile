   ...
   (when admin?
     [:<>
      [quo/list-item {:theme    :accent
                      ...
                      :on-press #(rf/dispatch [:community.member/add-role community-id public-key constants/community-member-role-moderator])}]])])
  ...
  (let [{:keys [community-id]} (rf/sub [:get-screen-params])]
    (fn []
      (let [my-public-key               (rf/sub [:multiaccount/public-key])
            {:keys [permissions
                    can-manage-users?
                    admin]}             (rf/sub [:communities/community community-id])
