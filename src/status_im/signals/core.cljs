  ...
  (let [{:keys [type event]} (types/json->clj event-str)]
    (case type
      ...
      "mailserver.request.completed" (when (accounts.db/logged-in? cofx)
                                       (mailserver/log-event [:mailserver.request.completed event])
      ...
      "mailserver.request.expired"   (when (accounts.db/logged-in? cofx)
                                       (mailserver/log-event [:mailserver.request.expired event])
                                       (mailserver/resend-request cofx {:request-id (:hash event)}))
      "discovery.summary" (do
                            (mailserver/log-event [:discovery.summary event])
                            (summary cofx event))
