                                            ...
                                            [network-preferences/view
                                             {:blur              false
                                              ...
                                              :on-save           (rn/use-callback
                                                                  (fn [chain-ids]
                                                                    (set-selected-networks (map wallet-common-utils/id->network chain-ids))
...
(defn view
  ...
  (let [address                                   (rf/sub [:wallet/wallet-send-to-address])
        ...
        [address-color set-address-color]         (rn/use-state (if (seq (:colorId saved-address))
                                                                  ...
                                                                  :blue))
        [selected-networks set-selected-networks] (rn/use-state
                                                   (when (seq (:chainShortNames saved-address))
                                                     (wallet-common-utils/network-preference-prefix->network-names (:chainShortNames saved-address))))
        selected-networks-or-fallback             (map (fn [network-name]
                                                         {:network-name network-name
                                                          :short-name   (wallet-common-utils/network->short-name network-name) })
                                                       (if (seq selected-networks)
                                                         selected-networks constants/default-network-names))
                                                          ...
                                                          (rf/dispatch
                                                           [:wallet/save-address
                                                            {:address             address
                                                             ...
                                                             :chain-short-names   (wallet-common-utils/short-names->network-preference-prefix
