(ns status-im.contexts.wallet.events
  (:require
    ...
    [status-im.contexts.wallet.tokens.db :as tokens.db]
     ...
     {:db (-> db
              ...
              ((fn [db]
                 (reduce (fn [db address]
                         ...
                         addresses))))
      :fx [[:dispatch [:wallet/get-last-wallet-token-update-if-needed]]
           ;; NOTE: fetching the market values and prices for tokens with balance separately, so the
           ;; user can see their balances quicker. Fetching this for all the tokens can be slower.
           [:dispatch [:wallet.tokens/fetch-market-values symbols]]
...
(rf/reg-event-fx :wallet/reload
 (fn [{:keys [db]}]
   (let [symbols (tokens.db/get-token-symbols db)]
