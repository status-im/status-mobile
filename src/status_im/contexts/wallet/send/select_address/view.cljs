...
(defn- address-input
  ...
  (let [set-scanned-value-fn (atom nil)]
    (fn []
      (let [view-id                  (rf/sub [:view-id])
        ...
        (when (and @set-scanned-value-fn scanned-address)
          (@set-scanned-value-fn scanned-address)
          (reset! set-scanned-value-fn nil))
        [quo/address-input
         {:on-focus              #(reset! input-focused? true)
          :on-blur               #(reset! input-focused? false)
          :on-scan               (fn [set-value]
                                   (reset! set-scanned-value-fn set-value)
                                   (rn/dismiss-keyboard!)
          ...
          :on-detect-address     #(when (or (= view-id :screen/wallet.select-address)
                                            (= view-id :screen/wallet.scan-address))
          ...
          :on-detect-ens         (fn [text cb]
                                   (when (or (= view-id :screen/wallet.select-address)
                                             (= view-id :screen/wallet.scan-address))
                                     ; ^ this check is to prevent effect being triggered when screen
                                     ; is loaded but not being shown to the user (deep in the
                                     ; navigation stack) and avoid undesired behaviors
