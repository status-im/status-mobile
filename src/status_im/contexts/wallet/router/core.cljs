...
(defn required-native-balance-amount
  [route]
  (let [{:keys [from-chain required-native-balance]} route
        native-token-decimals                        (:native-currency-decimals from-chain)]
    (-> required-native-balance
        (number/convert-to-whole-number native-token-decimals)
        money/bignumber)))

(schema/=> required-native-balance-amount
  [:=>
   [:cat router.schema/?route]
   :schema.common/big-number])
