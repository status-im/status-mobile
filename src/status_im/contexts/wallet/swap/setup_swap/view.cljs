...
(defn- alert-banner
  ...
  (let [error-response         (rf/sub [:wallet/swap-error-response])
        ...
        enough-native-balance? (rf/sub [:wallet/swap-enough-native-balance?])
        error-text             (cond
                                 pay-input-error? (i18n/label :t/insufficient-funds-for-swaps)
                                 (not enough-native-balance?)
                                 (swap-utils/error-message-from-code
                                  constants/router-error-code-not-enough-native-balance)
                                 :else (swap-utils/error-message-from-code error-response-code
                                                                           error-response-details))
        native-balance-error?  (or (not enough-native-balance?)
                                   (= error-response-code
                                      constants/router-error-code-not-enough-native-balance))
