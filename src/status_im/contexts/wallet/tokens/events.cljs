...
(defn store-token-list
  ...
  (let [chain-ids                  (networks.db/get-chain-ids db)
    ...
    {:fx [[:dispatch [:wallet.tokens/fetch-market-values symbols]]
          [:dispatch [:wallet.tokens/fetch-details symbols]]
          [:dispatch [:wallet.tokens/fetch-prices symbols]]]
     :db (-> db
             (assoc-in [:wallet :tokens]
                       {:sources                    (:sources tokens)
                        ...
                        :symbols                    symbols})
             (assoc-in [:wallet :ui :loading :token-list] false))}))
...
(rf/reg-event-fx :wallet.tokens/fetch-market-values
 (fn [{:keys [db]} [token-symbols]]
   (let [profile-currency (profile.db/get-currency db)]
     {:db (assoc-in db [:wallet :ui :loading :market-values] true)
...
(rf/reg-event-fx :wallet.tokens/fetch-details
 (fn [{:keys [db]} [token-symbols]]
   {:db (assoc-in db [:wallet :ui :loading :details] true)
    :fx [[:effects.wallet.tokens/fetch-details
          {:symbols    token-symbols
...
(rf/reg-event-fx :wallet.tokens/fetch-prices
 (fn [{:keys [db]} [token-symbols]]
   (let [profile-currency (profile.db/get-currency db)]
     {:db (assoc-in db [:wallet :ui :loading :prices] true)
      :fx [[:effects.wallet.tokens/fetch-prices
            {:symbols    token-symbols
             ;; NOTE: fetching the USD currency as well, since the number of token decimals
             ;; we show the user depends on the token USD price.
             :currencies [profile-currency constants/usd-currency]
