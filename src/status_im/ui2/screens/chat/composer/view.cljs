...
(defn get-y-value
  [context min-y max-y added-value max-height chat-id suggestions reply]
         ...
         [:f>
          (fn []
            (let [reply (<sub [:chats/reply-message])
                  ...
                  max-y (- window-height
                           (if (> keyboard-height 0) keyboard-height 360)
                           (:top insets)
                           (:status-bar-height @navigation-const)) ; 360 - default height
                  max-height (Math/abs (- max-y 56 (:bottom insets))) ; 56 - top-bar height
                  added-value (if (and (not (seq suggestions)) (or edit reply)) 38 0) ; increased height
                                                                                      ; of input box
                                                                                      ; needed when reply
                  ...
                  input-content-change (get-input-content-change context
                                                                 translate-y
                                                                 shared-height
                                                                 max-height
                                                                 set-bg-opacity
                  ...
                  bottom-sheet-gesture (get-bottom-sheet-gesture context
                                                                 translate-y
                                                                 text-input-ref
                                                                 keyboard-shown
                                                                 min-y
              ...
              [reanimated/view
               ...
               (when-not (seq suggestions)
                 [rn/view {:style (styles/bottom-sheet-controls insets)}
                  ...
                  [rn/view
                   {:ref   send-ref
                    :style (when-not (seq (get @input/input-texts chat-id))
                             {:width 0
                              :right -100})}
                   [quo2.button/button
                    {:icon                true
