...
(rf/defn handle-request-to-join [{:keys [db]} r]
...
(rf/defn handle-removed-chats [{:keys [db]} chat-ids]
...
(rf/defn handle-response [_ response-js]
...
(rf/defn reset-community-id-input [{:keys [db]} id]
  {:db (assoc db :communities/community-id-input id)})

(rf/defn reset-channel-info [{:keys [db]}]
...
(rf/defn create-channel-pressed
  ...
  (rf/merge cofx
            (reset-community-id-input id)
            (reset-channel-info)
            (rf/dispatch [::create-channel-fields (rand-nth emoji-thumbnail-styles/emoji-picker-default-thumbnails)])
