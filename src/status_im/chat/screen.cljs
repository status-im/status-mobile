...
(views/defview toolbar-action []
  (views/letsubs [show-actions? [:chat-ui-props :show-actions?]]
    [react/touchable-highlight
     {:on-press            #(re-frame/dispatch [:chat/set-chat-ui-props {:show-actions? (not show-actions?)}])
    ...
    (let [contacts' (contacts-by-identity contacts)
      ...
      [react/list-view
       {:renderRow                 (fn [row _ index]
        ...
        :onEndReached              (when-not loaded? #(re-frame/dispatch [:chat/load-more-messages]))
...
(views/defview chat []
  (views/letsubs [group-chat [:chat :group-chat]
    ...
    {:component-did-mount    #(do (re-frame/dispatch [:chat/check-and-open-dapp!])
     ...
     :component-will-unmount #(re-frame/dispatch [:chat/set-chat-ui-props {:show-emoji? false}])}
    [react/view {:style     st/chat-view
                 :on-layout (fn [event]
                              (let [height (.. event -nativeEvent -layout -height)]
                                (when (not= height layout-height)
                                  (re-frame/dispatch [:chat/set-layout-height height]))))}
     [chat-toolbar]
