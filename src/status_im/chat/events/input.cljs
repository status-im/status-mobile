...
(defn select-chat-input-command
  ...
  [{{:keys [current-chat-id chat-ui-props] :as db} :db}
...
(defn- request-command-data
  ...
  (let [{:keys [dapp? dapp-url name]} (get contacts chat-id)
    ...
    (commands-events/jail-request-data db request-data data-type)))
...
(handlers/register-handler-fx
  ...
  (fn [{:keys [db]} [command metadata prevent-auto-focus?]]
    (select-chat-input-command {:db db} command metadata prevent-auto-focus?)))
...
(handlers/register-handler-fx
  ...
  (fn [_ [{:keys [markup validationHandler parameters]} proceed-events]]
    (let [error-events-creator (fn [validator-result]
                                 [[:chat/set-chat-ui-props {:validation-messages validator-result
          ...
          events (cond
                   ...
                   [[::execute-validation-handler
                    ...
                    [:chat/set-chat-ui-props {:sending-in-progress? false}]]
