...
(rf/defn clean-input [{:keys [db] :as cofx} current-chat-id]
  (rf/merge cofx
            {:db (-> db
...
(rf/defn send-messages [{:keys [db] :as cofx} input-text current-chat-id]
...
(rf/defn send-edited-message [{:keys [db] :as cofx} text {:keys [message-id quoted-message]}]
  (rf/merge
   cofx
