(ns status-im.protocol.protocol-handler
  (:require [status-im.utils.logging :as log]
            ...
            [status-im.models.chats :refer [inactive-group-chats
                                            active-group-chats]]))
...
(defn make-handler [db]
  {:ethereum-rpc-url ethereum-rpc-url
   ...
   :inactive-groups  (inactive-group-chats)
   :storage          kv/kv-store
   :handler          (fn [{:keys [event-type] :as event}]
                       (case event-type
                         ...
                         :new-group-chat (dispatch [:group-chat-invite-received event])
                         ...
                         :group-new-participant (dispatch [:participant-invited-to-group event])
                         ...
                         :participant-left-group (dispatch [:participant-left-group event])
