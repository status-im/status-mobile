(ns status-im2.contexts.communities.context-drawers.community-options.view
  (:require [i18n.i18n :as i18n]
            [utils.re-frame :as rf]
            [quo2.core :as quo]
            [status-im2.contexts.communities.context-drawers.menus.leave.view :as leave-menu]))
...
(defn view-rules [id] {:icon                :i/bullet-list
...
(defn mute-community [id]
  (let [muted? (rf/sub [:communities/community id])
        muted-length 15]
    {:icon                (if muted? :i/unmuted :i/muted)
     :accessibility-label (if muted? :unmute-community :mute-community)
     :label               (i18n/label :t/mute-community)
     :sub-label           (when muted? (str "muted for " muted-length "minutes"))
     :right-icon          :i/chevron-right}))

(defn community-notification-settings [id]
...
(defn cancel-request-to-join [id]
  {:icon     :i/block
   :label    (i18n/label :t/cancel-request-to-join)
   :danger?      true
   :on-press            #(js/alert (str "implement action" id))})

(defn edit-community [id]
  {:icon     :i/edit
   :label    (i18n/label :t/edit-community)
   :on-press            #(js/alert (str "implement action" id))})

(defn not-joined-options [id token-gated?]
...
(defn join-request-sent-options
  [id token-gated?]
  [(conj (first (not-joined-options id token-gated?)) (cancel-request-to-join id))])

(defn join-request-denied
  [id token-gated?]
