(ns status-im2.contexts.onboarding.enable-notifications1.view
  (:require
   [quo2.core :as quo]
  ...
  (let [profile-color (:color (rf/sub [:onboarding-2/profile]))]
    [rn/view {:style (style/buttons insets)}
     ...
     [quo/button
      {:on-press                  (fn []
                                    ;; (shell.animation/change-selected-stack-id :communities-stack true)
                                    (rf/dispatch [:navigate-to :enable-notifications2]))
...
(defn enable-notifications
  ...
  (let [insets (safe-area/get-insets)]
    [rn/view {:style (style/page-container insets)}
     ...
     [parallax/video
      {:layers (resources/get-parallax-video :notifications1)}]
     [rn/view
