...
(defn drag-gesture
  [height saved-height opacity bg-bottom window-height keyboard-shown max-height input-ref lines add-keyboard-height saved-keyboard-height emojis-open gesture-enabled? last-height]
            ...
            (fn []
              (let [{:keys [keyboard-shown keyboard-height]} (hooks/use-keyboard)
                ...
                (when (and (not kb-default-height) (pos? keyboard-height))
                  (rf/dispatch [:kb-default-height keyboard-height]))
                [:<>
