...
@pytest.mark.xdist_group(name="one_to_one_chat_multiple")
class TestOneToOneChatMultipleSharedDevices(MultipleSharedDeviceTestCase):
    ...
    @marks.critical
    def test_reactions_to_message_in_1_1_chats(self):
    ...
    @marks.high
    def test_edit_delete_message_in_one_to_one_chat(self):
        ...
        if not self.home_1.get_pn(message_to_delete).is_element_disappeared(30):
    ...
    def test_push_notification_1_1_chat_no_pn_activity_center(self):
        ...
        self.device_2.just_fyi("Send emoji message to Device 1 while it's on backround")
...
class TestMessagesOneToOneChatMultiple(MultipleDeviceTestCase):
    ...
    def test_unread_messages_counter_preview_highlited_1_1_chat(self):
        ...
        home_1.home_button.double_click()
        if not home_1.get_chat(default_username_2).chat_preview.is_element_differs_from_template(
...
@pytest.mark.xdist_group(name="one_to_one_chat_single")
class TestOneToOneChatSingleSharedDevice(MultipleSharedDeviceTestCase):

    @classmethod
    def setup_class(cls):
        cls.drivers, cls.loop = create_shared_drivers(1)
        cls.driver = cls.drivers[0]
        cls.sign_in = SignInView(cls.driver)
        cls.home = cls.sign_in.create_user()
        cls.chat = cls.home.add_contact(basic_user['public_key'])
        cls.chat_name = basic_user['username']

    @marks.testrail_id(5387)
    @marks.high
    def test_delete_one_to_one_chat_via_delete_button(self):
        message = self.home.get_random_message()
        self.chat.send_message(message)
        self.home.just_fyi("Deleting chat via delete button and check it will not reappear after relaunching app")
        self.chat.delete_chat()
        self.chat.get_back_to_home_view()
