...
class Account(object):
    ...
    @property
    def nonce(self):
        return w3.eth.get_transaction_count(self.account_address)

    @property
    def balance(self):
        return w3.eth.get_balance(self.account_address)

    def send_eth(self, to_address, eth_value, gas_price_increment=0):
        signed_txn = w3.eth.account.sign_transaction(dict(
            nonce=self.nonce,
            gasPrice=w3.eth.gas_price + gas_price_increment * 1000000000,
            gas=21000,
            to=to_address,
...
class ContractInteractions(object):
    ...
    def transfer_token_to(self, from_address, to_address, number_of_tokens, nonce, gas_price_increment=0):
        gas_price = w3.eth.gas_price + gas_price_increment * 1000000000
...
def current_gas_price():
    return str(w3.eth.gas_price / 1000000000)
...
#     return getattr(w3.eth.get_transaction(hash), param)
#
